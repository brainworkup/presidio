...
resource = Resource(
    attributes={SERVICE_NAME: "pii-demo-app", "service.namespace": "pii-demo"}
)
...
def generate_user_registration_log():
    ...
    with tracer.start_as_current_span("user-registration") as span:
        ...
        unredacted_msg = (
            f"User registered: {name}, SSN: {ssn}, Phone: {phone}, Email: {email}"
        )
...
def generate_address_update_log():
    ...
    # UNREDACTED VERSION - Contains PII
    unredacted_msg = (
        f"Address updated for {name}, New address: {address}, Contact: {phone}"
    )
    logger.info(f"[UNREDACTED] {unredacted_msg}")
...
def generate_support_ticket_log():
    ...
    # UNREDACTED VERSION - Contains PII
    unredacted_msg = (
        f"Support ticket created by {name} (Email: {email}, Phone: {phone})"
    )
